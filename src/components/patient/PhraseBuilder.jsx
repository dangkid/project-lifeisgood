...
export default function PhraseBuilder({ selectedButtons, onRemoveButton, onClear, voiceGender = 'female', profileId, userId = 'default' }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t-4 border-blue-600 dark:border-blue-400 shadow-2xl z-40 transition-colors">
      <div className="max-w-7xl mx-auto p-3 sm:p-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 flex-wrap max-h-32 sm:max-h-40 overflow-y-auto">
          {selectedButtons.map((button, index) => (
            <div
              key={`${button.id}-${index}`}
              className="relative group bg-blue-50 dark:bg-blue-900/20 border-4 border-blue-300 dark:border-blue-600 rounded-xl px-3 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-3 touch-manipulation transition-colors"
            >
              {button.image_url && (
                <img 
              ...
              <span className="text-base sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">
                {button.text}
              </span>
              <button
                onClick={() => onRemoveButton(index)}
                className="ml-1 sm:ml-2 p-2 rounded-full bg-red-500 hover:bg-red-600 active:bg-red-700 text-white transition-colors touch-manipulation"
              >
        ...
        <div className="flex gap-3 sm:gap-4">
          <button
            ...
            className={`
              ...
              ${ isSpeaking 
                ? 'bg-green-600 dark:bg-green-700 animate-pulse' 
                : 'bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-800 active:bg-blue-800 dark:active:bg-blue-900'
              }
          ...
          <button
            onClick={onClear}
            className="px-4 sm:px-6 py-4 sm:py-6 rounded-2xl bg-red-600 dark:bg-red-700 hover:bg-red-700 dark:hover:bg-red-800 active:bg-red-800 dark:active:bg-red-900 text-white font-bold transition-colors flex items-center gap-2 sm:gap-3 touch-manipulation shadow-lg hover:shadow-xl text-xl sm:text-2xl border-4 border-transparent active:border-white"
          >
